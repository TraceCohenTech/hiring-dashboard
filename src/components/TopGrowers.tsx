...
import { Trophy, Brain, ArrowUpRight } from 'lucide-react';
...
export default function TopGrowers({ data }: TopGrowersProps) {
  ...
  return (
    <div ref={ref}>
      <SectionHeader
        title="Top Growers"
        subtitle={`Top ${data.length} companies by net jobs added`}
        icon={<Trophy size={18} />}
      />
      <motion.div
        ...
        {data.map((company, i) => {
          ...
          return (
            <motion.div
              ...
              className="grid grid-cols-[2rem_1fr_6rem_5.5rem_5.5rem] md:grid-cols-[2rem_1fr_1fr_6rem_5.5rem_5.5rem] gap-x-3 px-5 py-3 border-b border-slate-50 hover:bg-slate-50/60 transition-colors items-center"
            >
              {/* Rank */}
